...
import { Check, ClipboardList, Clock, RefreshCw, XCircle, ChevronRight, ChevronLeft, MessageSquare, Shield, Lock, FileText, AlertTriangle, MessageSquareText, Users, Eye, UserCog } from "lucide-react";
...
export default function AdminPanel({ reports, adminRole, adminEmail, adminDivision, onUpdateStatus, onAddComment, onRefreshReports }: AdminPanelProps) {
  ...
  const [staffName, setStaffName] = useState("Mas Handri (FTI)");
        ...
        <div className={`lg:col-span-5 space-y-4 ${selectedReportId ? "hidden lg:block" : "block"}`}>
          <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm space-y-4">
            ...
            <div className="flex flex-wrap gap-1 bg-slate-100 p-1 rounded-xl">
              {["ALL", ReportStatus.MENUNGGU, ReportStatus.DIPROSES, ReportStatus.SELESAI].map((status) => (
                <button
                  key={status}
                  onClick={() => {
              ...
              <div className="p-6 space-y-6">
                {isPimpinan && (
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <span>Anda login sebagai Pimpinan — akses baca saja. Tindak lanjut (ubah status, disposisi, balas resmi) hanya dapat dilakukan oleh Moderator, Staff, atau Super Admin.</span>
                  </div>
                )}
